...
// runKeyNew is the testable core of "cifra key new".
	...
	if vault.IsInitialized(repoRoot) {
		...
		if err := vault.AddRecipient(repoRoot, r); err != nil {
			if errors.Is(err, vault.ErrRecipientAlreadyExists) {
				ui.Info("recipient    already in .cifra/recipients — skipped")
			} else {
				ui.Warn("could not add to .cifra/recipients: " + err.Error())
			}
		} else {
			ui.Info("recipient    added to .cifra/recipients")
		}
	}
	return nil
}
...
func newKeyListCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "list",
		Short: "List all recipients in .cifra/recipients",
		RunE: func(cmd *cobra.Command, _ []string) error {
			wd, err := os.Getwd()
			if err != nil {
				return fmt.Errorf("get working directory: %w", err)
...
// runKeyList is the testable core of "cifra key list".
func runKeyList(repoRoot string) error {
	...
	if len(recipients) == 0 {
		ui.Info("no recipients found in .cifra/recipients")
		return nil
	}

	ui.Header(fmt.Sprintf("%d recipient(s) in .cifra/recipients", len(recipients)))
...
// runKeyExport is the testable core of "cifra key export --public".
...
func newKeyImportCmd() *cobra.Command {
	return &cobra.Command{
		...
		Short: "Add a teammate's public key to .cifra/recipients",
...
// runKeyImport is the testable core of "cifra key import <id> <hex-pubkey>".
...
func newKeyDeleteCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "delete",
		Short: "Remove a keypair from the OS keychain (and from .cifra/recipients)",
		RunE: func(cmd *cobra.Command, _ []string) error {
			kc, err := openKeychain()
			if err != nil {
				return err
	...
	cmd.Flags().BoolVar(&keepRecipient, "keep-recipient", false, "do not remove the entry from .cifra/recipients")
	_ = cmd.MarkFlagRequired("id")
	return cmd
}

// runKeyDelete is the testable core of "cifra key delete".
func runKeyDelete(id string, kc keychain.Store, repoRoot string, keepRecipient bool) error {
	...
	if err := vault.RemoveRecipient(repoRoot, id); err != nil {
		...
		ui.Warn("could not remove from .cifra/recipients: " + err.Error())
		return nil
	}
	ui.Info("removed from .cifra/recipients")
	return nil
}
...
// runKeyReseal is the testable core of "cifra key reseal".
