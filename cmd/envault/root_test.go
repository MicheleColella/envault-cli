...
func TestRootCommandsRegistered(t *testing.T) {
	cmd := newRootCmd("dev")
	want := []string{"init", "key", "import", "add", "set", "rm", "list", "cat", "export", "push", "pull", "run", "hook"}
...
func TestStubCommandsReturnError(t *testing.T) {
	stubs := []string{"push", "pull", "run", "hook"}
...
func TestStubErrorContainsCommandNameAndReason(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "push") {
		t.Errorf("error = %q, want command name 'push'", err.Error())
	}
