...
func newAgentCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "agent",
		Short: "Manage the key-unlock agent (ssh-agent-style, for headless envault callers)",
		Long: "Caches your decrypted private key in memory for a bounded time so headless\n" +
			...
			"prompt for a passphrase — can use it without ENVAULT_PASSPHRASE.\n\n" +
...
func newAgentUnlockCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Finds your recipient identity the same way `envault run` does (scans this\n" +
...
func runAgentUnlock(repoRoot string, kc keychain.Store, ttl time.Duration) error {
	...
	ui.Info("Claude Code (and any other headless envault caller) can now use it without a passphrase prompt")
	return nil
}
...
func runAgentStatus() error {
	...
	if len(entries) == 0 {
		ui.Info("No agent running, or no keys unlocked. Use `envault agent unlock` to start one.")
		return nil
	}

	ui.Header("Envault Agent — unlocked identities")
